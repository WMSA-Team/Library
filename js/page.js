  ...
  function render (ar) {
    ...
    for (let i=0; i<4; i++){
      ...
      let card = document.createElement('div');
            ...
            p.innerText = `${ar[random].description.slice(0,99)}......`;
...
let bookDonate  = document.getElementById('bookDonate')
let modal  = document.getElementById('modal')
function donating (e) {
  e.preventDefault() ;
  console.log('hi')
  // console.log(e.target.namew.value)
  // if (e.target.id === "donate") {
    console.log('hiw')
    // console.log(e.target.namew.value,e.target.desc.value,e.target.img.value,e.target.type.value)
...
bookDonate.addEventListener('submit', donating)

function start () {
  if (localStorage.getItem('Book.arr') === null) {
    ...
    Book.arr = JSON.parse(localStorage.getItem('Book.arr'))
  }
}
start()
render(Book.arr);